...
class TrainedJEPAAdapter:
    ...
    def predict_with_uncertainty(self, ctx: ContextEmbedding, n_samples: int | None = None) -> tuple[list[float], float]:
        vectors = self._ctx_to_matrix(ctx)
        if not self.is_available:
            predicted = self._weighted_merge(vectors)
            return self._l2_normalize(predicted), 1.0
        ...
        variance = float(preds.var(dim=0).mean().item())
        uncertainty = max(0.0, min(1.0, variance))
        return mean.cpu().tolist(), round(uncertainty, 6)
    ...
    def _predict_response_embedding(self, ctx: ContextEmbedding) -> ResponsePrediction:
        predicted, uncertainty = self.predict_with_uncertainty(ctx, self.config.uncertainty_samples)
    ...
    def _extract_emotional_labels(self, text: str) -> list[str]:
        lowered = text.lower()
        return sorted({keyword for keyword in self.config.salience_keywords if keyword in lowered})
